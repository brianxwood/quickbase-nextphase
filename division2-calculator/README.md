...
- **Gear** — 6 slots: item (brand piece, gear set piece, named or exotic), core attribute,
  ...
  cores/talents are enforced from the item data.
...
`data.js` is regenerated rather than hand-edited; the build script lives in the commit history for
this directory. Two corrections were applied to the source data: mojibake in accented names and
dashes was repaired, and each specialization's "Signature Weapon Damage" perk was re-pointed from
the generic `weaponDamage` stat to `signatureWeaponDamage`.
